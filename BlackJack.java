...
public class BlackJack {
    ...
    private static void initializeDeck() {
        ...
        for (String suit : SUITS) {
            for (int i = 0; i < RANKS.length; i++) {
                int value = (i < 9) ? i + 2 : 10; // simplfied //Determines Values for cards 2-10, Jack, Queen, King - Harry
    ...
    private static int playerTurn(Scanner scanner, List<Card> playerHand) {
        while (true) {
            int playerTotal = calculateHandValue(playerHand); // Added to Calculate current hand value based on Aces being present - Connor
            ...
            if (action.equals("hit")) {
                ...
                if (calculateHandValue(playerHand) > 21) { // Changed to calculateHandValue(playerHand) to include Aces - Connor
                    //System.out.println("You busted! Dealer wins."); // Notify player if they bust
                    playerTotal = 0;
                    return calculateHandValue(playerHand); // End the turn and return the busted value // Changed to calculateHandValue(playerHand) to include Aces - Connor
                }
        ...
        return calculateHandValue(playerHand); // Return the player's total after their turn // Changed to calculateHandValue(playerHand) to include Aces - Connor
    }
